...
function replaceColorLiteral(match: string, color: string): string {
  ...
  if (alphaColorMapping[color]) {
    ...
    return `theme.tokens.${token} + '${Math.round(opacity * 255).toString(16).padStart(2, '0')}'`;
  }
...
function transformFile(filePath: string): { content: string; changes: number } {
  ...
  let newContent = content.replace(hexPattern, (match, hex) => {
    const fullHex = hex.length === 3 ? 
      hex.split('').map((c: string) => c + c).join('') : hex;
    const color = `#${fullHex}`;
 ...
 * 디렉토리 내 모든 파일 변환
 */
function transformDirectory(dirPath: string): { files: string[]; totalChanges: number } {
  const files: string[] = [];
  let totalChanges = 0;
